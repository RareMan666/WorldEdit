...
class DefaultBlockParser extends InputParser<BaseBlock> {
   ...
   private BaseBlock parseLogic(String input, ParserContext context) throws InputParseException, NoMatchException, DisallowedUsageException {
      ...
      if (context.isRestricted()
         ...
         && this.worldEdit.getConfiguration().disallowedBlocks.contains(blockId)) {
         ...
         switch (blockType) {
            ...
            case WALL_SIGN:
               String[] text = new String[]{
                  blockAndExtraData.length > 1 ? blockAndExtraData[1] : "",
                  blockAndExtraData.length > 2 ? blockAndExtraData[2] : "",
                  blockAndExtraData.length > 3 ? blockAndExtraData[3] : "",
                  blockAndExtraData.length > 4 ? blockAndExtraData[4] : ""
               };
