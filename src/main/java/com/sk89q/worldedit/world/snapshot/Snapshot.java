...
public class Snapshot implements Comparable<Snapshot> {
   ...
   private ChunkStore internalGetChunkStore() throws IOException, DataException {
      if (this.file.getName().toLowerCase().endsWith(".zip")) {
         try {
            ...
            ChunkStore chunkStorex = new ZippedMcRegionChunkStore(this.file);
            return (ChunkStore)(!chunkStorex.isValid() ? new ZippedLegacyChunkStore(this.file) : chunkStorex);
         }
      } else if (!this.file.getName().toLowerCase().endsWith(".tar.bz2")
         && !this.file.getName().toLowerCase().endsWith(".tar.gz")
         && !this.file.getName().toLowerCase().endsWith(".tar")) {
   ...
   public boolean containsWorld(String worldname) {
      try {
         if (!this.file.getName().toLowerCase().endsWith(".zip")) {
            if (!this.file.getName().toLowerCase().endsWith(".tar.bz2")
               && !this.file.getName().toLowerCase().endsWith(".tar.gz")
               && !this.file.getName().toLowerCase().endsWith(".tar")) {
