...
public final class CraftBukkit_v1_6_R3 implements BukkitImplAdapter {
   ...
   private static Block getBlockFromTypeId(int type) {
      return type >= 0 && type < 4096 ? Block.byId[type] : null;
   }
   ...
   private Tag toNative(NBTBase foreign) {
      return this.toNative(getTagName(foreign.getTypeId()), foreign);
   }
      ...
      if (foreign == null) {
         ...
         if (foreign instanceof NBTTagByte) {
            ...
            try {
               ...
               this.logger.log(Level.WARNING, "Failed to convert NBTTagList", var9);
         ...
         for (Object obj : ((NBTTagCompound)foreign).c()) {
   ...
   private ListTag toNative(NBTTagList foreign) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
      ...
      for (int i = 0; i < foreign.size(); i++) {
         ...
         values.add(tag);
         cls = (Class<? extends Tag>)tag.getClass();
      }
   ...
   private NBTBase fromNative(String name, Tag foreign) {
      if (foreign == null) {
         ...
         for (Entry<String, Tag> entry : ((CompoundTag)foreign).getValue().entrySet()) {
            tag.set(entry.getKey(), this.fromNative(entry.getKey(), entry.getValue()));
         }
