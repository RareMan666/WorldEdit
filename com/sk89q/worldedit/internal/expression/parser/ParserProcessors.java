...
public final class ParserProcessors {
   ...
   private static RValue processUnaryOps(LinkedList<Identifiable> input) throws ParserException {
      ...
      while (!input.isEmpty()) {
         Identifiable last = input.removeLast();
         if (last instanceof OperatorToken) {
            ...
            if (!(last instanceof UnaryOperator)) {
               ...
               while (true) {
                  ...
                  lastx = input.removeLast();
                  lastPosition = lastx.getPosition();
                  if (!(lastx instanceof UnaryOperator)) {
                     break;
                  }

                  String operator = ((UnaryOperator)lastx).operator;
                  if (!operator.equals("+")) {
                     ...
                     try {
                        last = Operators.getOperator(lastPosition, opName, (RValue) last);
                     } catch (NoSuchMethodException var9) {
               ...
               if (lastx instanceof Token) {
                  throw new ParserException(lastPosition, "Extra token found in expression: " + lastx);
               }

               if (lastx instanceof RValue) {
                  throw new ParserException(lastPosition, "Extra expression found: " + lastx);
               }

               throw new ParserException(lastPosition, "Extra element found: " + lastx);
            }
