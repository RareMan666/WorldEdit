...
public class ClipboardCommands {
   ...
   public void copy(
      Player player, LocalSession session, EditSession editSession, @Selection Region region, @Switch('e') boolean copyEntities, @Switch('m') Mask mask
   ...
   public void rotate(Player player, LocalSession session, Double yRotate, @Optional Double xRotate, @Optional Double zRotate) throws WorldEditException {
      if (yRotate != null && Math.abs(yRotate % 90.0) > 0.001
         || xRotate != null && Math.abs(xRotate % 90.0) > 0.001
         || zRotate != null && Math.abs(zRotate % 90.0) > 0.001) {
         player.printDebug("Note: Interpolation is not yet supported, so angles that are multiples of 90 is recommended.");
      }

      ClipboardHolder holder = session.getClipboard();
      AffineTransform transform = new AffineTransform();
      transform = transform.rotateY(-(yRotate != null ? yRotate : 0.0));
      transform = transform.rotateX(-(xRotate != null ? xRotate : 0.0));
      transform = transform.rotateZ(-(zRotate != null ? zRotate : 0.0));
   ...
   public void flip(Player player, LocalSession session, EditSession editSession, @Optional("me") @Direction Vector direction) throws WorldEditException {
      ClipboardHolder holder = session.getClipboard();
      Clipboard clipboard = holder.getClipboard();
      AffineTransform transform = new AffineTransform();
