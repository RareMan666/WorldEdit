...
public class CuboidClipboard {
   ...
   private Vector size;
   private List<CuboidClipboard.CopiedEntity> entities = new ArrayList<>();
   ...
   public void flip(CuboidClipboard.FlipDirection dir, boolean aroundPlayer) {
      ...
      switch (dir) {
         case WEST_EAST:
            ...
            if (aroundPlayer) {
               this.offset = this.offset.setX(1.0 - this.offset.getX() - width);
            }
            break;
         case NORTH_SOUTH:
            ...
            for (int zs = 0; zs < len; zs++) {
               for (int x = 0; x < width; x++) {
                  for (int y = 0; y < height; y++) {
                     BaseBlock block1x = this.data[x][y][zs];
                     if (block1x != null) {
                        block1x.flip(dir);
                     }
                     ...
                     if (zs != length - zs - 1) {
                        ...
                        this.data[x][y][length - zs - 1] = block1x;
                     }
                  }
               }
            }
            ...
            if (aroundPlayer) {
               this.offset = this.offset.setZ(1.0 - this.offset.getZ() - length);
            }
            break;
         case UP_DOWN:
            ...
            for (int ys = 0; ys < hei; ys++) {
               for (int x = 0; x < width; x++) {
                  for (int z = 0; z < length; z++) {
                     BaseBlock block1xx = this.data[x][ys][z];
                     if (block1xx != null) {
                        block1xx.flip(dir);
                     }
                     ...
                     if (ys != height - ys - 1) {
                        ...
                        this.data[x][height - ys - 1][z] = block1xx;
                     }
                  }
               }
            }
            ...
            if (aroundPlayer) {
               this.offset = this.offset.setY(1.0 - this.offset.getY() - height);
            }
      }
   }
   ...
   public LocalEntity[] pasteEntities(Vector newOrigin) {
      ...
      for (int i = 0; i < this.entities.size(); i++) {
         CuboidClipboard.CopiedEntity copied = this.entities.get(i);
   ...
   public void storeEntity(LocalEntity entity) {
      this.entities.add(new CuboidClipboard.CopiedEntity(entity));
   }
